...
/* Added this - a process table. */

#define MAX_PROCESSES 50

struct process {
	int parent_pid;// not sure if needed, implement if needed
	int pid;
	int occupied; //initalize so that the process_table entry is available until set, if set to 1 KEEP THAT, dont change to 0
	int exited;	//set to 0 at the begininning
	int exit_code;
	struct thread * my_thread;
	struct semaphore *exit_sem;
};

struct process process_table[MAX_PROCESSES]; // DO NOT USE 0 or 1 elements for a process
...
struct thread {
	...
	char *t_stack;
	int thread_pid; // Added this field to store the PID of the thread because it corresponds to a process.

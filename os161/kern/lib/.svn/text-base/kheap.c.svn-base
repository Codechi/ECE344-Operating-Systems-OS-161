...
void *
kmalloc(size_t sz)	///kmalloc used for example like this:	child_tf  = (struct trapframe*) kmalloc(sizeof(struct trapframe));
{
	if (sz>=LARGEST_SUBPAGE_SIZE) {
		...
		//debugging
		//kprintf("IM IN KMALLOC and address is %u and hex representation is %x\n",address,address);
		return (void *)address;
	}

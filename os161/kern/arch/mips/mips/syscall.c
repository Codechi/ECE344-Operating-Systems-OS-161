...
mips_syscall(struct trapframe *tf) //trapframe definition in trapframe.h
{
	...
	//tf contains execution state of parent
	...
	case SYS_write:
		write_return = sys_write(tf->tf_a0,tf->tf_a1,tf->tf_a2,&retval);
		if(write_return <0)
 			err = 1;
 		else 
 			err = 0;
...
int sys_fork(struct trapframe *tf, int* returnval)
{
	...
	kprintf("In syscall.c: child_tf gets virtual address: %x\n",child_tf);
...
int md_forkentry(struct trapframe * child_tf, unsigned long converted_ptr_child_addrspace)
{
	...
	// kprintf("Entering md_forkentry for the %dth time\n", md_fork_entry); //USEFUL FOR TESTING, KEEP LATER
	 ...
	 struct trapframe child_frame_to_usermode =  *child_tf;//local_chiild_frame will have contents of child trap frame which is child_tf points to
	...
	//This should not be used by threads returning from traps - they
	 ...
	 mips_usermode(&child_frame_to_usermode);
...
void sys__exit(int code, int *gettingpid)
{
	//process_table[curthread->thread_pid].exited = 1; //set the exited flag for this process I htink not needed
	process_table[curthread->thread_pid].exit_code = code; //set the exit code for this process
	
	*gettingpid = curthread->thread_pid;//to get pid of thread that will exit
	
	/*FOR DEBUGGING
	kprintf("in sys__exit function in syscall.c: I am thread: %d exiting with status code %d and exited flag as %d",curthread->thread_pid,
	process_table[curthread->thread_pid].exit_code,process_table[curthread->thread_pid].exited);//USEFUL FOR TESTING, KEEP LATER
	*/
	
	//kprintf("Thread with pid %d is exiting with exit code %d\n",curthread->thread_pid,code);
...
int sys_waitpid(pid_t pid, int *returncode, int flags, int * returnval)//from unistd.h
{
	...
	if(returncode == NULL)	//check later iof ok
	{
		*returnval = EFAULT;
	...
	//kprintf("\nThread with %d going through.\n", curthread->thread_pid);

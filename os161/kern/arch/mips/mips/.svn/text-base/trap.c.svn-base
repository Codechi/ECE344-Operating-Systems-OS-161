 ...
 */
 ...
 // called mainly from syscall in __syscall in os161/lib/libc/syscalls-mips.S. 
 // but there are calls in between also. This function in kernel eventually calls mips_syscall(tf) and sends the trapframe
void
mips_trap(struct trapframe *tf)
{
	 ...
	 */
	 ...
	 //trapframe.h has exception codes  
	switch (code) {
	case EX_MOD:	/* EX_MOD - TLB Modify (write to read-only page) */
		if (vm_fault(VM_FAULT_READONLY, tf->tf_vaddr)==0) {
			goto done;
		}
		break;
	case EX_TLBL: /* EX_TLBL - TLB miss on load */
		if (vm_fault(VM_FAULT_READ, tf->tf_vaddr)==0) {
			goto done;
		}
		break;
	case EX_TLBS: /* EX_TLBS - TLB miss on store */

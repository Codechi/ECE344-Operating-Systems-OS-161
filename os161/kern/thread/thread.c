...
thread_create(const char *name)
{
	...
	if (thread==NULL) {
		return NULL;	//if no space, return NULL, when sets errno ENOMEM 
	}
	...
	//Iniitialize the PID by finding the first empty spot in the proces table array
	int i;
	for(i=2;i<MAX_PROCESSES;i++)
	{
		if (process_table[i].occupied == 0)//not occupied
		{
			//finds an empty index starting from 2 and sets the index's struct's pid to the index, my_thread pointing to the new created thread , and occupied to true
			thread->thread_pid = i;
			process_table[i].pid = thread->thread_pid;//or i
			process_table[i].my_thread = thread;
			process_table[i].occupied = 1;//occupied now
			//exitied set to 0 at initiialization
			//kprintf("This is %dth thread being recorded in process table where i starts from 2\n",i);//USEFUL FOR DEBUGGING BUT TESTER FAILS IF THIS THERE
			
			
			process_table[i].exit_sem = sem_create("exit_sem",1);
			P(process_table[i].exit_sem);
			
			number_of_user_threads++;//on first thread created, this becomes zero since starts from -1
			//kprintf("curent number of user threads: %d\n",number_of_user_threads);//USEFUL FOR DEBUGGING BUT TESTER FAILS IF THIS THERE
	...
	//may not be needed since returns NULL if cant be made
	if (i ==MAX_PROCESSES)
	{
		//errno = EAGAIN;
...
void initialize_process_table(void)
{
	...
	{
		process_table[i].my_thread = NULL;
		process_table[i].occupied = 0;	//struct process process_table[MAX_PROCESSES] declared in thread.h
		process_table[i].exited = 0;	//not exited
		
	}
	
	number_of_user_threads = -1;//since kernel makes a thread and sets it to 0
	kprintf("Initialized process table and number of user threads is %d\n",number_of_user_threads);
...
thread_exit(void)
{
	...
	//added in these 2 staments to make sure thead exits and releases lock
	process_table[curthread->thread_pid].exited = 1;
	V(process_table[curthread->thread_pid].exit_sem);
	number_of_user_threads--;//decrement number of user threads since this thread is exiting
